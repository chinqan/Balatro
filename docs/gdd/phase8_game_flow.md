...
> [!IMPORTANT]
> 地圖介面已重設計為 **Balatro 風格 3 張卡牌並排佈局**。舊版垂直節點樹已廢棄。

#### Balatro 風格 3 卡佈局

每一層（Floor）的所有遭遇固定顯示為 3 張卡牌（enc[0]、enc[1]、enc[2]），卡牌位置固定，黃框隨 `currentEncounter` 索引移動：

```
┌──────────────────────────────────────────────────────────────────────┐
│  F1 ● F2 ○ F3 ○ F4 ○ F5 ○ F6 ○ F7 ○ F8 ○          ❤️100 💰0 🔮0  │  ← 頂部 Header
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   ┌─────────────┐   ┌═════════════╗   ┌─────────────┐              │
...
| 卡牌狀態 | 標籤 Badge | 邊框 | 說明 |
|---------|----------|:----:|------|
| 已完成（`i < encIdx`）| `✓ 已完成` | 深灰 + 半透明遮罩 | 已擊敗，不可互動 |
| 當前（`i === encIdx`）| `Select`（金色）| **橘金色粗框** | 含行動按鈕，可操作 |
| 未來（`i > encIdx`）  | `Upcoming` | 暗藍灰 | 預覽資訊，不可互動 |
...
| 操作 | 方式 | 回饋 |
|------|------|------|
| 查看遭遇 | 所有遭遇資訊直接顯示在卡牌上，無需懸停/點擊 | HP / ATK / 勝利獎勵 / Boss 機制一覽 |
| 進入戰鬥 | 點擊 Current 卡內「⚔️ 開始戰鬥」藍色按鈕 | 場景切換至戰鬥 |
| 跳過遭遇 | 點擊 Current 卡內「⏩ 跳過遭遇」紅色按鈕 | 立即執行，無確認彈窗 |
| 查看 Boss | 點擊 Boss 卡圖示（Upcoming 時也會顯示 Boss 預覽蓋板） | Boss 詳細蓋板彈出 |

#### 跳過遭遇 — Inline 獎勵顯示

> [!IMPORTANT]
> **已廢棄確認彈窗**。跳過獎勵現在直接顯示在當前遭遇卡牌內，點擊「跳過遭遇」立即執行。
...
[ ⚔️ 開始戰鬥 ]              ← 藍色主按鈕
        or
┌────────────────────────┐
│ 💰 跳過獎勵：+4 金      │    ← 深紅邊框 Inline 預覽框
│   即時獲得金幣          │      (文字＋描述，隨遭遇類型而異)
└────────────────────────┘
[ ⏩ 跳過遭遇 ]              ← 紅色，點擊立即生效
```
...
| 跳過類型 | 放棄的 | 可能獎勵 | 機率 |
|---------|-------|----------|:----:|
| 跳過普通遭遇 | 3-5 金 + 商店 | 💰 +4 金 | 55% |
| 跳過普通遭遇 | 3-5 金 + 商店 | 🧪 消耗品槽 +1 | 30% |
| 跳過普通遭遇 | 3-5 金 + 商店 | 💰 +6 金（幸運） | 15% |
| 跳過菁英遭遇 | 5-7 金 + 商店 | 💰 +8 金 | 40% |
| 跳過菁英遭遇 | 5-7 金 + 商店 | 🧪 消耗品槽 +1 | 35% |
| 跳過菁英遭遇 | 5-7 金 + 商店 | 🔮 隨機遺物 | 25% |
